...
const noMessagingConfig = merge(config, {
    output: {filename: '[name]-no-messaging.js'},
    resolve: {alias: {'./protocol/chatProto.json': path.resolve(__dirname, 'src', 'sdk/protocol/shimProto.json')}}
});
...
const minifiedConfigs = [config, noMessagingConfig, noEdgeConfig, externalizedConfig].map(function(config) {
    return merge(config, {
...
module.exports = [config, noMessagingConfig, noEdgeConfig, externalizedConfig, reactNativeConfig, nodeConfig].concat(minifiedConfigs);